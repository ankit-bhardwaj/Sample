...
+(void)removeObjectForKey:(NSString*)fileNameKey {
    NSError* error = nil;
    NSFileManager* fileManager = [NSFileManager defaultManager];
    [fileManager removeItemAtPath:[[FTWCache cacheDirectory] stringByAppendingPathComponent:fileNameKey] error:&error];
    if (error) {
        NSLog(@"File removal not complete because of %@",[error localizedDescription]);
    }
}
