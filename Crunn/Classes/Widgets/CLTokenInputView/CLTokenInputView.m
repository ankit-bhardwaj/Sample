...
static CGFloat const TEXT_FIELD_HSPACE = 0.0; // Note: Same as CLTokenView.PADDING_X
...
- (void)commonInit
{
    ...
    self.bottomImageView = [[UIImageView alloc] initWithImage:[[UIImage imageNamed:@"blue_placeholder.png"] resizableImageWithCapInsets:UIEdgeInsetsMake(0, 5, 5, 5) resizingMode:UIImageResizingModeStretch]];
    [self insertSubview:self.bottomImageView atIndex:0];
    self.bottomImageView.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin;
    self.bottomImageView.frame = CGRectMake(0, 0, self.bounds.size.width, self.bounds.size.height-5);
