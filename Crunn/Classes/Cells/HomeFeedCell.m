...
-(void)fillDataWithTask:(Task*)atask
{
    ...
    self.assigneeImage.target = self;
    self.assigneeImage.action = @selector(assigneeImageTapped:);
    ...
    self.reminderBtn.hidden = !atask.HasReminder;
...
{
    [[APPDELEGATE window] makeToast:self.task.creator.Email duration:3.0 position:CSToastPositionCenter title:self.task.creator.FormattedName image:imageview.image];
}
