...
- (void)includeDate:(NSDate*)date
{
    if([_allDates containsObject:date])
    {
        KalTileView *preTile = [frontMonthView tileForDate:date];
        preTile.state = KalTileStateNone;
        [_allDates removeObject:date];
    }
    else
    {
        _beginDate = date;
        [_allDates addObject:date];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([hitView isKindOfClass:[KalTileView class]]) {
        ...
        if(self.selectionMode == KalSelectionModeMultiple)
        {
            [self includeDate:date];
        }
        else{
...
{
    if (self.selectionMode == KalSelectionModeSingle || self.selectionMode == KalSelectionModeMultiple)
        return;
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    if ([hitView isKindOfClass:[KalTileView class]]) {
        ...
        if (((self.selectionMode == KalSelectionModeSingle || self.selectionMode == KalSelectionModeMultiple) && tile.belongsToAdjacentMonth) ||
        ...
        if (self.selectionMode == KalSelectionModeRange)
        {
            ...
            if ([(id)delegate respondsToSelector:@selector(tapOnDate:)]) {
                [delegate tapOnDate:self.beginDate];
            }
        }
    }
}
